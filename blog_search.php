...
<body>
		...
		<?php 
		 ...
		 ?>
				...
				<a href="my_blog.php" class="btn btn-default btn-lg active" role="button" 
					style=" color: white; background: #006669;float: right;margin-top: 30px;height: 40px;font-size: 20px;
					 margin-right:12px; paddding:30px;">
                    My Blog</a>
<div class="page-wrapper">
			
	<div class="post-slider">
		<h1 class="slider-title" style="margin-left:8% !important;">Trending Post</h1>
		...
		<div class="post-wrapper">
					<?php	
				$query = "SELECT * FROM tbl_blog WHERE active=1 AND (likes=$maximum OR likes=$maximum2 OR likes=$maximum3 OR likes=$maximum4) AND likes!=0  order by id desc limit $start_from, $per_page";
				$post= $db->select2($query);
				while ($result = mysqli_fetch_array($post)){
        ?>
		<div class="post">
			<img src="<?php echo $result['image']; ?>" class="slider-image">
			<div class="post-info">
				<h3><a href="single.html"><?php echo $fm->textShorten2($result['title'],30);?></a></h3>
				<div style="font-size: 22px;margin: 5px; margin-bottom: 5px; color:#093c99;">
				<i class="fa fa-user "><a href="user_profile.php?user=<?php echo $result['user_name'];?>"> <?php echo $result['author'];?></a></i>
				<br>
				<i class="fa fa-calendar"><span style="margin-left: 5px;"><?php echo $fm->formatDate($result['date']);?></span></i>
				<i class="fa fa-heart" aria-hidden="true"> <?php echo' '.$result['likes']; ?></i>
	...
	<div class="content clearfix">
		...
		<div class="main-content">
			...
			<?php	
				$query = "SELECT * FROM tbl_blog WHERE active=1 AND title LIKE '%$search%' OR body LIKE '%$search%' ORDER BY id DESC LIMIT $start_from, $per_page";
...
<?php 
	...
	<a style="font-size: 30px; padding: 30px;" href="blog_search.php?page=<?php echo '1';?>"><?php echo '<'; ?></a>

	<?php for($i=1; $i<=$total_pages; $i++){?>
		<a style="padding: 2px;font-size: 30px;" href="blog_search.php?page= <?php echo $i;?>"><?php echo $i?></a>

	<?php }?>


	<a style="font-size: 30px; padding: 30px;" href="blog_search.php?page=<?php echo $total_pages;?>"><?php echo '>'; ?></a>
